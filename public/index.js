...
socket.on("temperature", (content) => {
  ...
  config.data.datasets.map((element, index) => {
    element.label == "Temperature"
      ? element.data.push(content.sensorData.temperature)
      : null;
    element.label == "Humidity"
      ? element.data.push(content.sensorData.humidity)
      : null;
  });
  window.myLine.update();
  let template =
    "<tr><td>" +
    content.sensorData.temperature +
    "ºC</td>" +
    "<td>" +
    content.sensorData.humidity +
    "%</td>" +
    "<td>" +
    content.time +
    "</td> </tr> ";
  ...
  document.getElementById("showTemperature").innerHTML =
    content.sensorData.temperature + "ºC";
...
document
  .getElementById("addTemperature")
  .addEventListener("click", function () {
    let result = false;
    config.data.datasets.map((element) => {
      if (element.label == "Temperature") result = true;
    });
    if (!result) {
      var newDataset = {
        label: "Temperature",
...
document.getElementById("addHumidity").addEventListener("click", function () {
  let result = false;
  config.data.datasets.map((element) => {
    if (element.label == "Humidity") result = true;
  });
  if (!result) {
    var newDataset = {
      label: "Humidity",
...
document
  .getElementById("removeTemperature")
  .addEventListener("click", function () {
    config.data.datasets.map((element, index) => {
      element.label == "Temperature"
